...
const READING_TEMPLATES = [
  (label) => `What is a key takeaway from ${label}?`,
  (label) => `According to ${label}, which statement holds true?`,
  (label) => `When reviewing ${label}, what should you keep in mind?`,
  (label) => `Which idea best reflects the guidance in ${label}?`,
];
...
function pickTemplate(type, label, idx) {
  ...
  if (type === 'reading') {
    return READING_TEMPLATES[idx % READING_TEMPLATES.length](label);
  }
...
function buildExplanation(type, label, fact) {
  ...
  if (type === 'reading') {
    return detail ? `${label} highlights that ${detail}` : `${label} underscores this idea for the exam.`;
  }
